...
export function HamburgerMenu(_props: { theme: string; toggleTheme?: () => void }) { void _props;
  ...
  return <><button className={`ham-btn ${open ? 'ham-open' : ''}`} onClick={() => setOpen(value => !value)} aria-label="Toggle menu"><span className="ham-line" /><span className="ham-line" /><span className="ham-line" /></button>{open && <div className="menu-backdrop" onClick={() => setOpen(false)} />}<nav className={`glass-menu ${open ? 'glass-open' : ''}`}><div className="glass-logo">Z·O</div><ul className="glass-links">{links.map(link => <li key={link.label}><a href={link.href} className="glass-link" onClick={() => setOpen(false)}><ChevronRight size={14} className="gl-arrow" />{link.label}</a></li>)}</ul><div className="glass-divider" />{/* Dark mode control temporarily hidden; keep available for later. */}{/* <button className="glass-theme" onClick={toggleTheme}>{theme === 'light' ? <><Moon size={15} /> Dark mode</> : <><Sun size={15} /> Light mode</>}</button> */}<div className="glass-socials"><a href="https://github.com/zackweb-pro" target="_blank" rel="noreferrer" aria-label="GitHub"><FaGithub size={18} /></a><a href="https://www.linkedin.com/in/zakaria-oumghar-gl" target="_blank" rel="noreferrer" aria-label="LinkedIn"><span>in</span></a><a href="https://x.com/zackweb_pro" target="_blank" rel="noreferrer" aria-label="X"><FaXTwitter size={18} /></a></div></nav></>;
}
